...
void keyPressed() {
  if (key == CODED) {
    if (keyCode == UP) player.isThrusting = true;
    if (keyCode == LEFT) player.isTurningLeft = true;
    if (keyCode == RIGHT) player.isTurningRight = true;
  }
}

void keyReleased() {
  if (key == CODED) {
    if (keyCode == UP) player.isThrusting = false;
    if (keyCode == LEFT) player.isTurningLeft = false;
    if (keyCode == RIGHT) player.isTurningRight = false;
  }
}
...
void draw() {
  ...
  //translate(targetX, height/2.5, 0);
  ...
  for (int x = 0; x < tex.width; x++) {
    ...
    for (int y = 0; y < tex.height; y++) {
      ...
      float bright = noise(xoff, yoff, zoff) * 80;
      //bright = 255;
      //fill(220, 180, 100);
      tex.pixels[x+y*width] = color(10, 10, 20, bright);
  ...
  pg = createGraphics(275, 275);



  pg.beginDraw();
  //pg.background(51); // Buffer background
  pg.fill(220, 180, 100); // Pink ellipse fill
  pg.noStroke();
  // Draw an ellipse at local buffer coordinates (200, 100) with size 80x80
  pg.ellipse(100, 100, 250, 250); 
  ...
  for (int x = 0; x < 800; x++) {
    ...
    for (int y = 0; y < 200; y++) {
      ...
      //bright = 255;
      //fill(220, 180, 100);
      pg.pixels[x+y*width] = color(220, 180, 100, bright);
      //tex.pixels[x+y*tex.width] = color(bright * 0.86, bright * 0.70, bright * 0.39);
      yoff += .01; // Increment y space
      // println(targetX);
    }
